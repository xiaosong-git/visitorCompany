...
public final class RedisUtil {
    ...
    private  JedisPool getJedisPool() throws Exception{
        if(jedisPool == null){
            ...
            property.load(is);
            String addr = property.getProperty("addr");
            String port = property.getProperty("port");
            String auth = property.getProperty("auth");
            is.close();
            ...
            jedisPool = new JedisPool(config, addr, Integer.valueOf(port), TIMEOUT, auth);
        }
        return jedisPool;
    }
     ...
     * @Date 2017/2/28 14:44
     */
    public static Long delObject(byte[] key, Integer dbNum) {
        Jedis jedis = null;
        try {
            jedis = getJedis(dbNum);
